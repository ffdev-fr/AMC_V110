...
TCPIPInitW5100:
   ...
   ldi   r16, TCPIP_SERVERSTATE_STOPPED
   sts   TCPIPServerState, r16
   ldi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
   sts   TCPIPSocketState0, r16
   sts   TCPIPSocketState1, r16
   ldi   r16, 0x00
   sts   TCPIPFailedNotifications, r16
...
;     .. Sockets 0 and 1 are initialized for TCPIP protocol
;     .. The 'PersistentSourcePort0' and 'PersistentSourcePort1 'ports are affected
;        to Socket 0 and 1
;     .. Interrupts are enabled for Sockets 0 and 1 events
;     .. The Sockets 0 and 1 are open and the listen state is entered
;     .. Sockets 0 and 1 are now waiting for client connection or termination of TCPIP server
;
; NOTES:
...
;  - The 'TCPIPFailedNotifications' shared variable is not resetted on TCPIP server start
;    (i.e. the application needs the number of restarts due to notification send failure)
TCPIPStartServer:

   ENTER_CRITICAL_SECTION             ; 'TCPIPStartServer' API in critical section
...
;
; This function is called by the 'TCPIPStartServer' API and the W5100 Interrupt Handler.
...
TCPIPProcessStartServer:
   ...
   ; NOTE: Server state is known by Socket 1 state (i.e. Socket 1 if always started
   ;       and closed after Socket 0  within a critical section)
   lds   r16, TCPIPSocketState1
...
TCPIPProcessStartServerStartRequired:
   ...
   ldi   XL, LOW(TCPIPNotificationFrame) ; Initialize Notification Frame (the 'TCPIPSendNotification'
                                         ; function will not change the ACP header and Frame type) 
   ...
   ldi   r16, ACP_FRAME_VERSION
   st    X+, r16                       
   ldi   r16, ACP_FRAME_NOTFICATION
   st    X+, r16                       
   
   ldi   r16, TCPIP_NOTIFICATIONSTATE_BUFFERRELEASED    ; Notification buffer ready
   sts   TCPIPNotificationState, r16

   ; 3- Set socket 0 port
   ; Socket 0 Source Port Register is located at addresses 0x0404-0x0405
	ldi 	r17, LOW(PersistentSourcePortLSB0)		; Pointer to ESEG address
	ldi 	r18, HIGH(PersistentSourcePortLSB0)
	call 	ReadPersistentByte
   mov   r18, r16
   ldi   r16, 0x04          ; Write in 0x0404 register
   ldi   r17, 0x04
   call  W5100WriteByte
	ldi 	r17, LOW(PersistentSourcePortMSB0)		; Pointer to ESEG address
	ldi 	r18, HIGH(PersistentSourcePortMSB0)
	call 	ReadPersistentByte
   mov   r18, r16
   ...
   ; 5- Set socket 1 port
   ; Socket 1 Source Port Register is located at addresses 0x0504-0x0505
	ldi 	r17, LOW(PersistentSourcePortLSB1)		; Pointer to ESEG address
	ldi 	r18, HIGH(PersistentSourcePortLSB1)
	call 	ReadPersistentByte
   mov   r18, r16
   ldi   r16, 0x04          ; Write in 0x0504 register
   ldi   r17, 0x05
   call  W5100WriteByte
	ldi 	r17, LOW(PersistentSourcePortMSB1)		; Pointer to ESEG address
	ldi 	r18, HIGH(PersistentSourcePortMSB1)
	call 	ReadPersistentByte
   mov   r18, r16
   ldi   r16, 0x05          ; Write in 0x0505 register
   ldi   r17, 0x05
   call  W5100WriteByte

   ; 6- Set socket 1 for TCPIP protocol
   ; Socket 1 Mode register address is 0x500
   ldi   r16, 0x00          ; Write in 0x0500 register
   ldi   r17, 0x05
   ldi   r18, 0b00000001    ; TCPIP protocol is bit0 set to 1
   call  W5100WriteByte
   ldi   r16, TCPIP_SOCKETSTATE_CLOSED
   ...
   ; 7- Enable interrupts for sockets 0 and 1
   ; Interrupt Mask Register address is 0x0016
   ldi   r16, 0x16          ; Write in 0x0016 register
   ldi   r17, 0x00
   ldi   r18, 0b00000011    ; Socket 0-1 interrupts enabled
   call  W5100WriteByte
   ...
   ; 9- Open socket 1
   ; The OPEN command is launched using the Socket 1 Command Register (address 0x0501)
   ; NOTE: The OPEN command should work at all times (i.e. status register not checked
   ;       after command launched)
   ldi   r16, 0x01          ; Write in 0x0501 register
   ldi   r17, 0x05
   ldi   r18, 0x01          ; OPEN command is 0x01
   call  W5100WriteByte
   ldi   r16, TCPIP_SOCKETSTATE_INIT
   ...
   ; 10- Start Listen on sockets 0 and 1
   ; The LISTEN command is launched using the Socket 0-1 Command Register (address 0x0401/0x0501)
   ...
   ; 11- From now any event on sockets 0-1 will trigger W5100 interrupt
   ...
   ldi   r16, TCPIP_SERVERSTATE_STARTED
   sts   TCPIPServerState, r16
...
;     .. Interrupts are disabled for Socket 0 and 1
;     .. If a connection is established, the DISCONNECT command is sent to disconnect
;        the peer from socket 0 and 1 (socket 0 and 1 state changed to CLOSED)
;     .. Otherwise, a CLOSE command id sent to Socket 0 and 1 (socket 0 and 1 should be open)
;     .. The Socket 0 and 1 state variable is updated (= 'TCPIP_SOCKETSTATE_NOTSTARTED')
;
; NOTE:
;  - The 'TCPIPStartServer' function can be called again to restart the server.
TCPIPStopServer:

   ENTER_CRITICAL_SECTION           ; 'TCPIPStopServer' API in critical section
   push  r16
   push  r17
   push  r18

   ; 1- Check if the server is started
   lds   r16, TCPIPSocketState0
   cpi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
   brne  TCPIPStopServer_StopRequired
   lds   r16, TCPIPSocketState1
   cpi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
   brne  TCPIPStopServer_StopRequired
...
TCPIPStopServer_StopRequired:
   ...
   ; 2- Disable interrupts for sockets 0 and 1
   ; Interrupt Mask Register address is 0x0016
   ldi   r16, 0x16          ; Write in 0x0016 register
   ldi   r17, 0x00
   ldi   r18, 0b00000000    ; Socket 0-1 interrupt disabled
   call  W5100WriteByte

   ; 3- Check if Socket 0 is open (should be open)
   lds   r16, TCPIPSocketState0
   cpi   r16, TCPIP_SOCKETSTATE_CLOSED
   breq  TCPIPStopServer_Socket0Closed
   ...
   brlo  TCPIPStopServer_CloseSocket0
   ...
   call  W5100WriteByte
   jmp   TCPIPStopServer_Socket0Closed

TCPIPStopServer_CloseSocket0:
...
TCPIPStopServer_Socket0Closed:

   ; 6- The socket 0 is stopped
   ; Update the 'TCPIPSocketState0' state variable
   ldi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
   sts   TCPIPSocketState0, r16

   ; 7- Check if Socket 1 is open (should be open)
   lds   r16, TCPIPSocketState1
   cpi   r16, TCPIP_SOCKETSTATE_CLOSED
   breq  TCPIPStopServer_Socket0Closed
   cpi   r16, TCPIP_SOCKETSTATE_CLOSING
   ...
   ; 8- If a connection is established, disconnect the peer
   cpi   r16, TCPIP_SOCKETSTATE_ESTABLISHED
   brlo  TCPIPStopServer_CloseSocket1

   ; Send DISCONNECT command to Socket 1
   ; The DISCONNECT command is launched using the Socket 1 Command Register (address 0x0501)
   ; NOTE: The DISCONNECT command should work at all times (i.e. status register not checked
   ;       after command launched)
   ldi   r16, 0x01          ; Write in 0x0501 register
   ldi   r17, 0x05
   ldi   r18, 0x08          ; DISCONNECT command is 0x08
   call  W5100WriteByte
   jmp   TCPIPStopServer_Socket1Closed

TCPIPStopServer_CloseSocket1:

   ; 9- If connection not established, close the socket (i.e. stop to listen)
   ; Send CLOSE command to Socket 1
   ; The CLOSE command is launched using the Socket 1 Command Register (address 0x0501)
   ; NOTE: The CLOSE command should work at all times (i.e. status register not checked
   ;       after command launched)
   ldi   r16, 0x01          ; Write in 0x0501 register
   ldi   r17, 0x05
...
TCPIPStopServer_Socket1Closed:

   ; 10- The socket 1 is stopped
   ; Update the 'TCPIPSocketState1' state variable
   ; 
   ; NOTE: Server state is known by Socket 1 state (i.e. Socket 1 if always started
   ;       and closed after Socket 0  within a critical section)
   ldi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
...
; Function for restarting the TCPIP server.
;
; Typically this function is called when the TCPIP server has reached the 
; 'DISCONNECTED' in order to launch a new server's life cycle.
TCPIPRestartServer:
   call  TCPIPStopServer
   call  TCPIPStartServer
   ret

; Function called to know if the TCPIP server is disconected.
;
; The 'DISCONNECTED' state is reached when an established connection to the server 
; is lost:
;  - A socket disconnection event has been received (typically when client closes 
;    the socket on termination)
;  - Failure to send a notification or a reply (i.e. socket send timeout).
;
; The function returns the disconnection state in R16 register:
;  - 0x00 = The TCPIP server has not been disconnected (or never connected).
;  - 0x01 = The TCPIP server is in the 'DISCONNECTED' state.
; 
; NOTE:
;  - Typically this function is periodically called by the application to detect
;    disconnection.
;  - When the TCPIP server is in the 'DISCONNECTED', the 'TCPIPRestartServer' function
;    must be called to restart the server.
TCPIPCheckServerDisconnected:

   ENTER_CRITICAL_SECTION             ; State checks are made in critical section
   lds   r16, TCPIPServerState
   LEAVE_CRITICAL_SECTION

   cpi   r16, TCPIP_SERVERSTATE_DISCONNECTED
   breq  TCPIPCheckServerDisconnected_Disconnected
   ldi   r16, 0x00
   rjmp  TCPIPCheckServerDisconnected_Exit

TCPIPCheckServerDisconnected_Disconnected:
   ldi   r16, 0x01

TCPIPCheckServerDisconnected_Exit:
   ret
...
TCPIPSendReply:
   ...
   ENTER_CRITICAL_SECTION      ; Access automaton state in critical section
   lds   r16, TCPIPSocketState0
   LEAVE_CRITICAL_SECTION
...
; Function for sending a Notification Frame to the client.
;
; This function works as follows:
;  - The caller function must set the Notification data in the 'TCPIPNotificationFrame'
;    buffer before calling.
;  - The function checks the TCPIP automaton state and Socket 1 TX buffer free size.
;  - The function copies the contents of the 'TCPIPNotificationFrame' buffer in the W5100 
;    Socket 1 TX buffer.
;  - The function launches a send operation on Socket 1.
;  - The fuction updates the TCPIP automaton state and returns.
;
; The function returns the result of the operation in R16 register:
;  - 0x01 = Notification send launched
;  - 0x00 = Cannot send Reply (wrong state, TX buffer full...)
;           Typically, the caller must retry later.
; 
; NOTE: 
;  The 'SendNotification' operation is synchronized by the 'TCPIPNotificationState' shared
;  variable (i.e. critical section for access to 'TCPIPNotificationFrame' buffer):
;   - The client function must check the 'TCPIPNotificationFrame' availability by reading the
;     value of the 'TCPIPNotificationState' shared variable in a critical section.
;   - If the buffer is available, the client function locks the buffer for its send operation
;     by setting the 'TCPIPNotificationState' to 'Buffer Acquired'.
;   - Then the client function populate the 'TCPIPNotificationFrame' and call the 
;     'TCPIPSendNotification' function.
;   - The 'TCPIPSendNotification' function will update the 'TCPIPNotificationState' variable
;     once 'TCPIPNotificationFrame' data are copied to the W5100 TX buffer.
;   - If the client function cannot acquire the 'TCPIPNotificationFrame' buffer, it must retry
;     later (typically on next interrupt).
;
; NOTE: 
;  All the Notification frame bytes are sent in a single call (i.e. the function fails if there
;  is not enough space in TX buffer to send the whole frame).
TCPIPSendNotification:
   ...
   ; 1- Check TCPIP automaton state
   ; This function is not called directly from the TCPIP automaton (i.e. W5100 interrupt handler).
   ; TCPIP automaton state is checked for design consistency (if implementation is clean, there is not reason
   ; to call this function in the wrong state).
   ENTER_CRITICAL_SECTION                ; Access automaton state in critical section
   lds   r16, TCPIPSocketState1
   cpi   r16, TCPIP_SOCKETSTATE_ESTABLISHED   ; Send notification allowed only if socket connected and idle
   breq  TCPIPSendNotification_CheckTXFreeSize
   LEAVE_CRITICAL_SECTION
   ldi   r16, 0x00                            ; Wrong state, not allowed to send notification
   jmp   TCPIPSendNotification_Exit

TCPIPSendNotification_CheckTXFreeSize:
   ; 2- Copy the 'Notification Frame' bytes in the TX buffer
   
   ; Enter the 'SENDINGNOTIF' state
   ldi   r16, TCPIP_SOCKETSTATE_SENDINGNOTIF
   sts   TCPIPSocketState1, r16
   ldi   r16, TCPIP_NOTIFICATIONSTATE_SENDINGBUFFER
   sts   TCPIPNotificationState, r16
   LEAVE_CRITICAL_SECTION
   
   ; Obtain the free size available in TX buffer
   ; Information available in Socket 1 TX Free Size Register (register address 0x0520-0x0521)
   ; The address '0x0520' must be read first (MSB of free size)
   ldi   r16, 0x20             ; Read in 0x0520 register
   ldi   r17, 0x05
   call  W5100ReadByte         ; r18 contains MSB of S1_TX_FSR on return
   mov   r20, r18
   inc   r16
   call  W5100ReadByte         ; r18 contains LSB of S1_TX_FSR on return
   mov   r19, r18              ; Free size stored in r20:r19

   ; Check if enough space in TX buffer for send of the 'Notification Frame'
   ; NOTE: By design the TX buffer cannot be satured
   cpi   r20, 0x00             ; Maximum 'Frame' size is 255 bytes
   brne  TCPIPSendNotification_ComputeOffset
   ldi   r16, TCPIPFrameSize
   cp    r16, r19              ; Carry set if r19 > r16
   brcc  TCPIPSendNotification_ComputeOffset

   ldi   r16, 0x00             ; Not enough space in TX memory to send notification
   jmp   TCPIPSendNotification_Exit

TCPIPSendNotification_ComputeOffset:

   ; Obtain the Write Pointer
   ; Information available in Socket 1 TX Write Pointer Register (register address 0x0524-0x0425)
   ; The address '0x0524' must be read first (MSB of read pointer)
   ; NOTE: 
   ;  - The current Write Pointer value is stored on stack
   ;  - This value will be used at the end of copy process to update the Write Pointer in the W5100
   ldi   r16, 0x24             ; Read in 0x0524 register
   ldi   r17, 0x05
   call  W5100ReadByte         ; r18 contains MSB of S1_TX_WR on return
   mov   r22, r18
   push  r18                   ; Save current value of 0x0524 register
   inc   r16
   call  W5100ReadByte         ; r18 contains LSB of S1_TX_WR on return
   mov   r21, r18
   push  r18                   ; Save current value of 0x0525 register

   ; Compute address offset for writing data
   andi  r21, LOW(TCPIPSocket1TxMask)     ; Address offset in r22:r21
   andi  r22, HIGH(TCPIPSocket1TxMask)   

   ; Source buffer ('TCPIPNotificationFrame') is addressed with XH:XL pointer
	ldi   XL, LOW(TCPIPNotificationFrame)
	ldi   XH, HIGH(TCPIPNotificationFrame)
   ...
   ldi   r18, 0x00
   adc   r17, r18
   cpi   r17, HIGH(TCPIPSocket1TxSize)
   brlo  TCPIPSendNotification_JumpNoOverflow
   brne  TCPIPSendNotification_Overflow
   cpi   r16, LOW(TCPIPSocket1TxSize)
   brlo  TCPIPSendNotification_JumpNoOverflow
   breq  TCPIPSendNotification_JumpNoOverflow
   rjmp  TCPIPSendNotification_Overflow

TCPIPSendNotification_JumpNoOverflow:
   jmp   TCPIPSendNotification_NoOverflow
      
TCPIPSendNotification_Overflow:
   ...
   ; 
   ; Bytes are written using the sequential access mode (i.e. if more than one byte to send)
   ; Compute Socket TX buffer start address (S1_TX_Base + Send data address Offset)
   ldi   r16, LOW(TCPIPSocket1TxBase)
   ldi   r17, HIGH(TCPIPSocket1TxBase)
   add   r16, r21
   adc   r17, r22

   ; Loop counter (i.e. number of bytes to write) is r24:r23
   ; Compute the number of bytes to write at the upper address
   ; Upper size is S1_TX_Size - Offset
   ldi   r23, LOW(TCPIPSocket1TxSize)
   ldi   r24, HIGH(TCPIPSocket1TxSize)
   sub   r23, r19
   sbc   r24, r20
   push  r23               ; Save the number of bytes to write at the upper address
   push  r24
   ...
   ; Check for only one byte to write in the upper memory block
   cpi   r24, 0x00
   brne  TCPIPSendNotification_OverflowStartUpperLoop
   cpi   r23, 0x01
   brne  TCPIPSendNotification_OverflowStartUpperLoop
   ...
   ld    r18, X                 ; Load r18 with the byte to write
   call  W5100WriteByte         
   jmp   TCPIPSendNotification_OverflowInitLowerLoop

TCPIPSendNotification_OverflowStartUpperLoop:
   ; More than one byte to write in the upper memory block
   ld    r18, X+                    ; Load r18 with the byte to write
   call  W5100WriteFirstByte    
   ...
   ldi   r16, 0x00                  ; Decrement loop counter
   dec   r23
   brne  TCPIPSendNotification_OverflowUpperLoop
   cpi   r24, 0x00
   breq  TCPIPSendNotification_OverflowInitLowerLoop
   dec   r24

TCPIPSendNotification_OverflowUpperLoop:
   cpi   r23, 0x01                  ; Is last byte to write
   brne  TCPIPSendNotification_OverflowUpperLoopWrite
   cpi   r24, 0x00
   brne  TCPIPSendNotification_OverflowUpperLoopWrite
   ldi   r17, 0x01                  ; Last byte to write

TCPIPSendNotification_OverflowUpperLoopWrite:
   ld    r16, X+                    ; Load r16 with the byte to write
   call  W5100WriteNextByte
   ...
   breq  TCPIPSendNotification_OverflowInitLowerLoop
   dec   r24
   jmp   TCPIPSendNotification_OverflowUpperLoop

TCPIPSendNotification_OverflowInitLowerLoop:
   ; Compute the number of bytes to write at the lower address
   ; The number of left bytes to write is: 
   ; number of bytes to send - number of bytes written at upper memory address 
   pop   r16                     ; Retrieve the number of bytes written at the upper address
   ...
   ; Bytes are written from the Socket 1 TX base address
   ldi   r16, LOW(TCPIPSocket1TxBase)
   ldi   r17, HIGH(TCPIPSocket1TxBase)
   ...
   ; Check for only one byte to write in the lower memory block
   cpi   r24, 0x00
   brne  TCPIPSendNotification_OverflowStartLowerLoop
   cpi   r23, 0x01
   brne  TCPIPSendNotification_OverflowStartLowerLoop
   ...
   ld    r18, X+                    ; Load r18 with the byte to write
   call  W5100WriteByte
   jmp   TCPIPSendNotification_BytesWritten

TCPIPSendNotification_OverflowStartLowerLoop:
   ; More than one byte to write in the lower memory block
   ld    r18, X+                    ; Load r18 with the byte to write
   call  W5100WriteFirstByte 
   ...
   ldi   r17, 0x00                  ; Decrement loop counter
   dec   r23
   brne  TCPIPSendNotification_OverflowLowerLoop
   cpi   r24, 0x00
   breq  TCPIPSendNotification_BytesWritten
   dec   r24

TCPIPSendNotification_OverflowLowerLoop:
   cpi   r23, 0x01                  ; Is last byte to write
   brne  TCPIPSendNotification_OverflowLowerLoopWrite
   cpi   r24, 0x00
   brne  TCPIPSendNotification_OverflowLowerLoopWrite
   ldi   r17, 0x01                  ; Last byte to write

TCPIPSendNotification_OverflowLowerLoopWrite:
   ld    r16, X+                    ; Load r16 with the byte to write
   call  W5100WriteNextByte
   ...
   breq  TCPIPSendNotification_BytesWritten
   dec   r24
   jmp   TCPIPSendNotification_OverflowLowerLoop

#endif


TCPIPSendNotification_NoOverflow:
   ; Bytes to send copied to a contiguous data block
...
TCPIPSendNotification_WriteNextByte:
   ld    r18, X+                    ; Load r18 with the byte to write
   call  W5100WriteByte
   ...
   ; Decrement loop counter
   dec   r23
   brne  TCPIPSendNotification_WriteNextByte
   cpi   r24, 0x00
   breq  TCPIPSendNotification_BytesWritten
   dec   r24
   jmp   TCPIPSendNotification_WriteNextByte
   ...
   ldi   r17, 0x00                  ; Decrement loop counter
   dec   r23
   brne  TCPIPSendNotification_NoOverflowLoop
   cpi   r24, 0x00
   breq  TCPIPSendNotification_BytesWritten
   dec   r24

TCPIPSendNotification_NoOverflowLoop:
   cpi   r23, 0x01                  ; Is last byte to read
   brne  TCPIPSendNotification_NoOverflowLoopWrite
   cpi   r24, 0x00
   brne  TCPIPSendNotification_NoOverflowLoopWrite
   ldi   r17, 0x01                  ; Last byte to read

TCPIPSendNotificationNoOverflowLoopWrite:
   ld    r16, X+                    ; Load r16 with the byte to write
   call  W5100WriteNextByte
   ...
   breq  TCPIPSendNotification_BytesWritten
   dec   r24
   jmp   TCPIPSendNotification_NoOverflowLoop

#endif


TCPIPSendNotification_BytesWritten:
   ; 3 - Bytes to send are ready in the TX buffer
   ; Launch the SEND command in the W5100
   
   ; Increase the Write Pointer value with the number of bytes to send
   ; Information available in Socket 1 TX Write Pointer Register (register address 0x0524-0x0525)
   ; The address '0x0524' must be read first (MSB of read pointer)
   ; NOTE: 
   ;  - The current Write Pointer value have been stored on stack
   pop   r19                   ; Current register 0x0525 value of S1_TX_WR in r19 (LSB)
   pop   r18                   ; Current register 0x0524 value of S1_TX_WR in r19 (MSB)
   ldi   r16, TCPIPFrameSize   ; Frame Size bytes written in TX buffer
   add   r19, r16
   ldi   r16, 0x00
   adc   r18, r16

   ldi   r16, 0x24             ; Write in 0x0524 register
   ldi   r17, 0x05
   call  W5100WriteByte        
   mov   r18, r19              ; Write in 0x0525 register
   inc   r16
   call  W5100WriteByte        
    
   ; The SEND command is launched using the Socket 1 Command Register (address 0x0501)
   ; Note: The W5100 SEND command is launched in a critical section because the
   ;       state and shared variables must be updated before receiving the next
   ;       TCPIP event.
   ; Note: The socket state is already 'SENDINGREPLY' (i.e. to protect this function
   ;       against re-entry)
   ENTER_CRITICAL_SECTION
   ldi   r16, TCPIP_NOTIFICATIONSTATE_BUFFERRELEASED     ; The notification buffer is available now
   sts   TCPIPNotificationState, r16
   ...
   call  W5100WriteByte
   LEAVE_CRITICAL_SECTION
   ldi   r16, 0x01          ; Send command successfully launched

TCPIPSendNotification_Exit:
...
TCPIPSocketEvent:
   ...
   ENTER_CRITICAL_SECTION         ; W5100 Interrupt Handler in critical section
   push  r16
   push  r17
   push  r18
   ...
   ; only socket 0-1 interrupts)
   ; Read Interrupt Register to check the source(s) of the interrupt
   ; Interrupt Register address is 0x0015
   ldi   r16, 0x15             ; Read in 0x0015 register
   ldi   r17, 0x00
   call  W5100ReadByte
   push  r18                              ; Save Interrupt Register value
	andi  r18, 0b00000001                  ; Is socket 0 interrupt
	brne  TCPIPSocketEvent_Socket0Event
   jmp   TCPIPSocketEvent_Socket1Event
...
TCPIPSocketEvent_CheckDisconnectEvent0:
   ...
   cpi   r16, TCPIP_SOCKETSTATE_ESTABLISHED
   breq  TCPIPSocketEvent_ProcessDisconnectEvent0        
...
TCPIPSocketEvent_CheckReceiveEvent0:
   ; Receive event is valid in 'TCPIP_SOCKETSTATE_ESTABLISHED' and
   ; 'TCPIP_SOCKETSTATE_RECEIVING' states
   cpi   r16, TCPIP_SOCKETSTATE_ESTABLISHED
   breq  TCPIPSocketEvent_ProcessReceiveEvent0        
   cpi   r16, TCPIP_SOCKETSTATE_RECEIVING
   breq  TCPIPSocketEvent_ProcessReceiveEvent0        
   jmp   TCPIPSocketEvent_ClearSocket0Int
             
TCPIPSocketEvent_CheckTimeoutEvent0:
   ; Timeout event is valid in 'TCPIP_SOCKETSTATE_RECEIVING' and
   ; 'TCPIP_SOCKETSTATE_SENDINGREPLY' states
   cpi   r16, TCPIP_SOCKETSTATE_RECEIVING
   breq  TCPIPSocketEvent_ProcessTimeoutEvent0        
   cpi   r16, TCPIP_SOCKETSTATE_SENDINGREPLY
   breq  TCPIPSocketEvent_ProcessTimeoutEvent0        
   jmp   TCPIPSocketEvent_ClearSocket0Int

TCPIPSocketEvent_CheckSentEvent0:
   ; Send OK event is valid in 'TCPIP_SOCKETSTATE_SENDINGREPLY' state
   cpi   r16, TCPIP_SOCKETSTATE_SENDINGREPLY
   breq  TCPIPSocketEvent_ProcessSentEvent0        
   jmp   TCPIPSocketEvent_ClearSocket0Int

   ; 3- Process Socket 0 event

TCPIPSocketEvent_ProcessConnectEvent0:
   ; 3.1- Process Incoming Connection event
   push  r18                           ; Save S0_IR flags (required to acknowledge the interrupt)
   ldi   r16, 0x00                     ; Event on Socket 0
   call  TCPIPProcessConnectEvent
   pop   r18
   jmp   TCPIPSocketEvent_ClearSocket0Int

TCPIPSocketEvent_ProcessDisconnectEvent0:
   ; 3.2- Process Disconnect event
   push  r18                           ; Save S0_IR flags (required to acknowledge the interrupt)
   call  TCPIPProcessDisconnectEvent
   pop   r18
   jmp   TCPIPSocketEvent_ClearSocket0Int

TCPIPSocketEvent_ProcessReceiveEvent0:
...
TCPIPSocketEvent_ProcessTimeoutEvent0:
   ; 3.4- Process Timeout event
   push  r18                           ; Save S0_IR flags (required to acknowledge the interrupt)
   call  TCPIPProcessTimeoutEvent
   pop   r18
   jmp   TCPIPSocketEvent_ClearSocket0Int

TCPIPSocketEvent_ProcessSentEvent0:
   ; 3.5- Process Send OK event
   push  r18                           ; Save S0_IR flags (required to acknowledge the interrupt)
   call  TCPIPProcessReplySentEvent
   pop   r18
...
TCPIPSocketEvent_ClearSocket0Int:
   ...
   ; 4- Check if Socket 1 event is signaled
   pop   r18                              ; Retrieve Interrupt Register value
   push  r18                              ; Save it again
	andi  r18, 0b00000010                  ; Is socket 1 interrupt
	brne  TCPIPSocketEvent_Socket1Event
   jmp   TCPIPSocketEvent_ClearUnexpectedInterrupt

TCPIPSocketEvent_Socket1Event:
   ; Socket 1 event is identified
   ; Read Socket 1 Interrupt Register to identify the event
   ; Socket 1 Interrupt Register address is 0x0502
   ldi   r16, 0x02             ; Read in 0x0502 register
   ldi   r17, 0x05
   call  W5100ReadByte         ; r18 contains S1_IR on return

   ; 5- Check the Socket 1 event according to socket state
   lds   r16, TCPIPSocketState1                    
   cpi   r18, 0b00000001       ; Is Incoming Connection event?
   breq  TCPIPSocketEvent_CheckConnectEvent1
   cpi   r18, 0b00000010       ; Is Disconnect event?
   breq  TCPIPSocketEvent_CheckDisconnectEvent1
   cpi   r18, 0b00000100       ; Is Receive event?
   breq  TCPIPSocketEvent_CheckReceiveEvent1
   cpi   r18, 0b00001000       ; Is Timeout event?
   breq  TCPIPSocketEvent_CheckTimeoutEvent1
   cpi   r18, 0b00010000       ; Is Send OK event?
   breq  TCPIPSocketEvent_CheckSentEvent1
   jmp   TCPIPSocketEvent_ClearSocket1Int     ; This point should never be reached
                                    
TCPIPSocketEvent_CheckConnectEvent1:
   ; Connect event is valid only in 'TCPIP_SOCKETSTATE_LISTENING' state                                 
   cpi   r16, TCPIP_SOCKETSTATE_LISTENING
   breq  TCPIPSocketEvent_ProcessConnectEvent1        
   jmp   TCPIPSocketEvent_ClearSocket1Int
             
TCPIPSocketEvent_CheckDisconnectEvent1:
   ; Disconnect event is valid in 'TCPIP_SOCKETSTATE_ESTABLISHED' and
   ; 'TCPIP_SOCKETSTATE_SENDINGNOTIF' states                                 
   cpi   r16, TCPIP_SOCKETSTATE_ESTABLISHED
   breq  TCPIPSocketEvent_ProcessDisconnectEvent1        
   cpi   r16, TCPIP_SOCKETSTATE_SENDINGNOTIF
   breq  TCPIPSocketEvent_ProcessDisconnectEvent1        
   jmp   TCPIPSocketEvent_ClearSocket1Int
             
TCPIPSocketEvent_CheckReceiveEvent1:
   ; Receive event should never occur.
   ; The Socket 1 is used only to send notifications (the master should never sent
   ; data to this socket)
   jmp   TCPIPSocketEvent_ClearSocket1Int
             
TCPIPSocketEvent_CheckTimeoutEvent1:
   ; Timeout event is valid in 'TCPIP_SOCKETSTATE_SENDINGNOTIF' states
   cpi   r16, TCPIP_SOCKETSTATE_SENDINGNOTIF
   breq  TCPIPSocketEvent_ProcessTimeoutEvent1        
   jmp   TCPIPSocketEvent_ClearSocket1Int

TCPIPSocketEvent_CheckSentEvent1:
   ; Send OK event is valid in 'TCPIP_SOCKETSTATE_SENDINGNOTIF' state
   cpi   r16, TCPIP_SOCKETSTATE_SENDINGNOTIF
   breq  TCPIPSocketEvent_ProcessSentEvent1        
   jmp   TCPIPSocketEvent_ClearSocket1Int

   ; 6- Process Socket 1 event

TCPIPSocketEvent_ProcessConnectEvent1:
   ; 6.1- Process Incoming Connection event
   push  r18                           ; Save S1_IR flags (required to acknowledge the interrupt)
   ldi   r16, 0x01                     ; Event on Socket 1
   call  TCPIPProcessConnectEvent
   pop   r18
   jmp   TCPIPSocketEvent_ClearSocket1Int

TCPIPSocketEvent_ProcessDisconnectEvent1:
   ; 6.2- Process Disconnect event
   push  r18                           ; Save S1_IR flags (required to acknowledge the interrupt)
   call  TCPIPProcessDisconnectEvent
   pop   r18
   jmp   TCPIPSocketEvent_ClearSocket1Int

TCPIPSocketEvent_ProcessTimeoutEvent1:
   ; 6.3- Process Timeout event
   push  r18                           ; Save S1_IR flags (required to acknowledge the interrupt)
   call  TCPIPProcessTimeoutEvent
   pop   r18
   jmp   TCPIPSocketEvent_ClearSocket1Int

TCPIPSocketEvent_ProcessSentEvent1:
   ; 6.4- Process Send OK event
   push  r18                           ; Save S1_IR flags (required to acknowledge the interrupt)
   call  TCPIPProcessNotifSentEvent
   pop   r18

TCPIPSocketEvent_ClearSocket1Int:
   ; The socket 1 interrupt has been processed, clear the interrupt register
   ; The S1_IR is cleared by writting one in register (for the interrupt(s) to acknowledge)
   ldi   r16, 0x02             ; Write in 0x0502 register
   ldi   r17, 0x05             ; r18 already contains the original S1_IR value
   call  W5100WriteByte
...
TCPIPSocketEvent_ClearUnexpectedInterrupt:
   ...
   ; Only bit5-7 may be set in IR (the other ones are socket and only sockets 0 and 1 in IMR).
   ; These interrupts are cleared by writing the corresponding bit in IR
   pop   r18                   ; Retrieve Interrupt Register value
   andi  r18, 0b11101100       ; Is any interrupt other than S0-S1 signaled
   breq  TCPIPSocketEvent_Exit
   ldi   r16, 0x15             ; Write in 0x0015 register
   ldi   r17, 0x00
...
;
; This function is called when a connect event is received on Socket 0 or 1.
; 
; The r16 register indicates the socket (0 = socket 0, 1 = socket 1)
; 
; This function does the following:
;  - For Socket 0
;     . The socket state variable 'TCPIPSocketState0' is updated to 'ESTABLISHED'
;     . The socket is now waiting for incoming data (a 'Command' frame) or disconnection.
;  - For Socket 1
;     . The socket state variable 'TCPIPSocketState1' is updated to 'ESTABLISHED'
;     . The socket is also ready to send data (i.e. a 'Notification' frame).
;  - Anytime
;     . If Socket 0 and Socket 1 state is 'TCPIP_SOCKETSTATE_ESTABLISHED', the TCPIP
;       server state is set to 'TCPIP_SERVERSTATE_CONNECTED'
; NOTE: 
...
TCPIPProcessConnectEvent:
   ...
   ldi   r17, TCPIP_SOCKETSTATE_ESTABLISHED
   cpi   r16, 0x01
   breq  TCPIPProcessConnectEvent_UpdateSocket1

   ; Update socket 0 state variable
   sts   TCPIPSocketState0, r17
   lds   r16, TCPIPSocketState1
   jmp   TCPIPProcessConnectEvent_UpdateServerState

TCPIPProcessConnectEvent_UpdateSocket1:
   ; Update socket 1 state variable
   sts   TCPIPSocketState1, r17
   lds   r16, TCPIPSocketState0

TCPIPProcessConnectEvent_UpdateServerState:
   ; Update server state if required
   cpi   r16, TCPIP_SOCKETSTATE_ESTABLISHED
   brne  TCPIPProcessConnectEvent_Exit
   ldi   r16, TCPIP_SERVERSTATE_CONNECTED
   sts   TCPIPServerState, r16

TCPIPProcessConnectEvent_Exit:
   ret

; Function called by 'TCPIPSocketEvent' interrupt handler for processing a disconnect event
;
; This function is called when a disconnect event is received on Socket 0 or 1.
; 
; The r16 register indicates the socket (0 = socket 0, 1 = socket 1)
; 
...
; The rules are: 
;  - If disconnected by peer the TCPIP server enters the 'DISCONNECTED' state.
;    Typically the application detects the 'DISCONNECTED' state and explictly restarts
;    the TCPIP server.
;  - If disconnected by 'TCPIPStopServer' the TCPIP server is really stopped
; 
; This function does the following:
;  - The TCPIP server state is updated to 'DISCONNECTED'
; 
; NOTE: 
;  - This function is called only if current socket state is valid for the operation
;  - Registers r16, r17, r18 are already saved by the caller function and this function
;    is allowed to modify them.
TCPIPProcessDisconnectEvent:

   ; Enter the 'DISCONNECTED' state (i.e. update server state)
   ; The server must be restarted in order to accept new incoming connection.
   ; This final state is used by the application to explicitly manage 'master' 
   ; sessions (connection/disconnection)
   ldi   r16, TCPIP_SERVERSTATE_DISCONNECTED
   sts   TCPIPServerState, r16
   ret 
...
;      .. The socket state variable 'TCPIPSocketState0' is updated to 'TCPIP_SOCKETSTATE_CMDRECEIVED'
...
;      .. The socket state variable 'TCPIPSocketState0' is updated to 'TCPIP_SOCKETSTATE_RECEIVING'
...
;
; This function is called when a send or disconnect command has timed out on Socket 0 or 1.
...
; 
; This function does the following:
;   - If the timeout occurs for a send command on socket 1, the 'TCPIPFailedNotifications'
;     counter is incremented.
;   - The sockets 0 and 1 are closed (forced close)
;   - The TCPIP server enters the 'DISCONNECTED' state (by design this event occurs only 
;     on a started server).
;     This is an abnormal situation and the server has to be explicitly restarted in order
...
TCPIPProcessTimeoutEvent:
   ...
   ; 1- Check if timeout occurs on socket 1 during a send notification operation
   cpi   r16, 0x01
   brne  TCPIPProcessTimeoutEvent_CloseSocket0
   lds   r16, TCPIPSocketState1
   cpi   r16, TCPIP_SOCKETSTATE_SENDINGNOTIF
   brne  TCPIPProcessTimeoutEvent_CloseSocket0
   lds   r16, TCPIPFailedNotifications
   inc   r16
   sts   TCPIPFailedNotifications, r16

TCPIPProcessTimeoutEvent_CloseSocket0:
   ; 2- Close the sockets
   ; Send CLOSE command to sockets
   ; The CLOSE command is launched using the Socket 0-1 Command Register (address 0x0401/0x0501)
   ; NOTE: The CLOSE command should work at all times (i.e. status register not checked
   ;       after command launched)
   lds   r16, TCPIPSocketState0                    ; Close Socket 0 if required
   cpi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
   breq  TCPIPProcessTimeoutEvent_CloseSocket1
   ldi   r16, 0x01                                 ; Write in 0x0401 register
   ldi   r17, 0x04
   ldi   r18, 0x10                                 ; CLOSE command is 0x10
   call  W5100WriteByte

   ldi   r16, TCPIP_SOCKETSTATE_NOTSTARTED    ; The socket 0 is stopped
   sts   TCPIPSocketState0, r16               ; Update the 'TCPIPSocketState0' state variable

TCPIPProcessTimeoutEvent_CloseSocket1:
   lds   r16, TCPIPSocketState1                    ; Close Socket 0 if required
   cpi   r16, TCPIP_SOCKETSTATE_NOTSTARTED
   breq  TCPIPProcessTimeoutEvent_RestartServer
   ldi   r16, 0x01                                 ; Write in 0x0501 register
   ldi   r17, 0x05
   ldi   r18, 0x10                                 ; CLOSE command is 0x10
   call  W5100WriteByte

   ldi   r16, TCPIP_SOCKETSTATE_NOTSTARTED    ; The socket 1 is stopped
   sts   TCPIPSocketState1, r16               ; Update the 'TCPIPSocketState1' state variable

TCPIPProcessTimeoutEvent_RestartServer:
   ; 3- Enter the 'DISCONNECTED' state (i.e. update server state)
   ; The server must be restarted in order to accept new incoming connection.
   ; This final state is used by the application to explicitly manage 'master' 
   ; sessions (connection/disconnection)
   ldi   r16, TCPIP_SERVERSTATE_DISCONNECTED
...
; This function does the following:
;   - The 'TCPIPSocketState0' variable is updated
...
; on Socket 1
;
; This function is called when all data have been sent on the Socket 1 (i.e. all data in
; the TX buffer of Socket 0 are sent).
; The send operation has been launched by call to 'TCPIPSendNotification' function.
; This function does the following:
;   - The 'TCPIPSocketState1' variable is updated
;
; NOTE: 
;  - This function is called only if current socket state is valid for the operation
;  - The function is called in a critical section
;  - Registers r16, r17, r18 are already saved by the caller function and this function
;    is allowed to modify them.
TCPIPProcessNotifSentEvent:

   ; Note: 'TCPIPSocketState1' shared variable accessed in critical section
   ;       (i.e. also checked by 'TCPIPSendNotification' which can be called by an 
   ;       interrupt handler) 

   ; Socket state changed from 'TCPIP_SOCKETSTATE_SENDINGNOTIF' to 'TCPIP_SOCKETSTATE_ESTABLISHED
   ldi   r16, TCPIP_SOCKETSTATE_ESTABLISHED
   sts   TCPIPSocketState1, r16
...
W5100Reset:
   ...
   push  r16
   ENTER_CRITICAL_SECTION      ; Interrupts not allowed during the reset
